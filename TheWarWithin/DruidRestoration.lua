...
spec:RegisterHook( "reset_precast", function ()
    ...
    if buff.casting.up and buff.casting.v1 == 740 then
        ...
        local tickInterval = spec.auras.tranquility.tick_time or 0
...
spec:RegisterAbilities( {
    ...
    pre_rejuvenation = {
        name = "回春术",
        listName = '|T134376:0|t |cff00ccff[回春术]|r',
        cast = 0,
        link = function () return ( GetSpellLink( 774 ) ) end,
        hot_id = 774 ,
        startsCombat = true,
        texture = 136081,
        cooldown = 0,
        ...
        handler = function ()
            ...
            if talent.germination.enabled then
                ...
                --self
                elseif buff.rejuvenation_germination.remains < buff.rejuvenation.remains then applyBuff( "rejuvenation_germination" ) --self
                end
    ...
    bear_wait= {
        name = "熊形态等待",
        listName = '|T134376:0|t |cff00ccff[熊形态等待]|r',
        indicator = "wait",
        texture = 132276,
        cast = 0,
        cooldown = 0,
    ...
    lifebloom = {
        id = function() return talent.undergrowth.enabled and 188550 or 33763 end,
        hot_id = function() return talent.undergrowth.enabled and 188550 or 33763 end,
        cast = 0,
        cooldown = 0,
        gcd = "spell",
    ...
    natures_cure = {
        ...
        usable = function ()
            return debuff.dispellable_magic.up or ((debuff.dispellable_curse.up or debuff.dispellable_poison.up) and talent.improved_natures_cure.enabled) or Hekili:isMouseOverMemberDispelable("Magic") or Hekili:isMouseOverMemberDispelable("Poison"), "requires magic, dispellable curse, potion"
        end,
    ...
    rejuvenation = {
        id = 774,
        --hot_id = function() return talent.germination.enabled and 188550 or 774 end,
        hot_id = 774 ,
        cast = 0,
        cooldown = 0,
        gcd = "spell",
    ...
    swiftmend = {
        ...
        usable = function ()
            local unit = find.lowest_hp.unit
            if not unit or unit == "none" then return false, "none target" end
            local _, _, _, _, _, _, _, _, _, regrowth_id = FindUnitBuffByID(unit, 8936)
            local _, _, _, _, _, _, _, _, _, rejuvenation_id = FindUnitBuffByID(unit, 774)
            local _, _, _, _, _, _, _, _, _, wild_growth_id = FindUnitBuffByID(unit, 48438)
            if regrowth_id or rejuvenation_id or wild_growth_id then
                return true
            else
                return false, "target doesnt have hots"
            end
        end,
    ...
    tranquility = {
        ...
        start = function()
            ...
            for i = 1, 4 do
                ...
                if tickTime <= query_time or 0 + spec.auras.tranquility.duration then
...
spec:RegisterSetting( "raid_rejuvenation_num", 8, {
    name = strformat( "团本%s数量", Hekili:GetSpellLinkWithTexture( spec.abilities.rejuvenation.id ) ),
    desc = strformat( "用于设置团本回春术的最大数量" ),
    type = "range",
    min = 0,
    max = 15,
    step = 1,
...
spec:RegisterSetting( "convoke_the_spirit_health", 75, {
    name = strformat( "%s团队血量设置", Hekili:GetSpellLinkWithTexture( spec.abilities.convoke_the_spirits.id ) ),
    desc = strformat( "当团队血量低于该设定值时，插件会推荐使用%s",Hekili:GetSpellLinkWithTexture( spec.abilities.convoke_the_spirits.id ) ),
    type = "range",
    min = 0,
    max = 100,
    step = 1,
...
spec:RegisterStateExpr( "convoke_the_spirit_health", function ()
    return settings.convoke_the_spirit_health or 75
end )

spec:RegisterSetting( "tranquility_health", 65, {
    name = strformat( "%s团队血量设置", Hekili:GetSpellLinkWithTexture( spec.abilities.tranquility.id ) ),
    desc = strformat( "当团队血量低于该设定值时，插件会推荐使用%s",Hekili:GetSpellLinkWithTexture( spec.abilities.tranquility.id ) ),
    type = "range",
    min = 0,
    max = 100,
    step = 1,
...
spec:RegisterStateExpr( "tranquility_health", function ()
    return settings.tranquility_health or 65
end )

spec:RegisterSetting( "flourish_health", 70, {
    name = strformat( "%s团队血量设置", Hekili:GetSpellLinkWithTexture( spec.abilities.flourish.id ) ),
    desc = strformat( "当团队血量低于该设定值时，插件会推荐使用%s",Hekili:GetSpellLinkWithTexture( spec.abilities.flourish.id ) ),
    type = "range",
    min = 0,
    max = 100,
    step = 1,
...
spec:RegisterStateExpr( "flourish_health", function ()
    return settings.flourish_health or 70
end )

spec:RegisterSetting( "incarnation_tree_of_life_health", 65, {
    name = strformat( "%s团队血量设置", Hekili:GetSpellLinkWithTexture( spec.abilities.incarnation_tree_of_life.id ) ),
    desc = strformat( "当团队血量低于该设定值时，插件会推荐使用%s",Hekili:GetSpellLinkWithTexture( spec.abilities.incarnation_tree_of_life.id ) ),
    type = "range",
    min = 0,
    max = 100,
    step = 1,
...
spec:RegisterStateExpr( "incarnation_tree_of_life_health", function ()
    return settings.flourish_health or 65
end )
...
spec:RegisterStateExpr( "combat_forbidden", function ()
    return settings.combat_forbidden or false
end )

spec:RegisterSetting("rejuvenation_forbidden", false, {
    name = strformat( "禁用%s", Hekili:GetSpellLinkWithTexture( spec.abilities.rejuvenation.id ) ),
    desc = "开启后则不再自动预铺回春术！需要用户手动提前预铺！",
    type = "toggle",
    width = "full",
} )
spec:RegisterStateExpr( "rejuvenation_forbidden", function ()
    return settings.rejuvenation_forbidden or false
end )

spec:RegisterPack( "恢复奶树Simc", 20250820, [[Hekili:fN1wVTXXv4Fl5LcLxiiPmTKascGCnSAmQeqalQH0dC5WDhsoM7fMzMvSsiGWxItSLVJyG6ABe7yy5OgiidJEZxK8FMYLK(j)xONzNLAND5UljDtrtaSL25mNlZ52354kfQ8hQu2aXXvwRy(ILYVyXc5kCY5N)elvPmFR24kLBJ0BHAa)GnYc(Z(x8PEp7ME7(p7)K7wMyPliylthKHGrmhxQoquhNoMUOkLR5sm5FPDLAjkJIfHB0gRxzTc5lvPCtIHbwslMb8TqHCf7E2UN17kVCW9E87F0fg(8l27ONm87)fV)1l7F774DTB2)Ax3BN96FV39HdVXGR(DE3(UGI9(3E)Hh8SEh(a53HR071p272VO3B3vE3bp)IdU3EEhC5EVDFjhE)3Vdq8)(cxQB1b)L3nCNx698l1)hFuVxDZEV(j9V8la2eWGdE3GJoO)vVZGh(37FRDHl59O9g(TpO3RUUVQkV2rpzWLEDV38DdVZngUZrF4WR6D7B49WFS)939dhETJ5v)D2r(XbV9UEh8aGr93)NaBuAiZn4HVY7OFO3BU17F63((F4nEhEbVR83(u42sUo3G93V3BUH3lVKKjXFFuSljb9F0l(uHocA5Z(5b)899E6L9E2FD4)469V1EdV5